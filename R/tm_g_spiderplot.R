      ...
      q1 <- teal.code::eval_code(
        ...
        code = bquote({
          plot <- osprey::g_spiderplot(
            ...
            line_color_opt = .(if (length(line_colorby_var) > 0 & all(unique(bquote(ANL_f[, .(line_colorby_var)])) %in% names(line_color_opt))) {
              line_color_opt
            } else {
              NULL
            }),
            marker_shape_opt = .(if (length(marker_var) > 0 & all(unique(bquote(ANL_f[, .(marker_var)])) %in% names(marker_shape_opt))) {
              marker_shape_opt
            } else {
              NULL
            }),
